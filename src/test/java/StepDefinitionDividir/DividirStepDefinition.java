...
public class DividirStepDefinition
{
    WebDriver victor;
    Hook hook=new Hook();
    DividirPOM dividir;
    ...
    public void ingreso_dos_numeros(DataTable dataTable) {
        ...
        dividir.IngresePrimerNumero(listaDatos.get(0));
        dividir.IngreseSegundoNumero(listaDatos.get(1));
        dividir.SeleccionePrototipo(listaDatos.get(2));
    ...
    public void selecciono_la_operacion(DataTable dataTable) {
        List<String> listaDatos=dataTable.asList(String.class);
        dividir.SeleccionOperacion(listaDatos.get(0));
    }
